...
import { FaComment, FaHeart, FaReply, FaPaperPlane, FaUser, FaMusic } from 'react-icons/fa';
...
export default function PlaylistComments({ playlistId, className = '' }) {
  ...
  const loadComments = useCallback(async () => {
    ...
    setError('');
    try {
      // Mock data for demo since backend might not be ready
      const mockComments = [
        {
          id: '1',
        ...
        {
          id: '2', 
          content: 'This brings back so many memories ❤️',
          userName: 'Nostalgic Fan',
          userAvatar: null,
      ...
      try {
        const realComments = await getPlaylistComments(playlistId);
  ...
  const handleSubmitComment = async (e) => {
    ...
    try {
      ...
      try {
        await addCommentToPlaylist(playlistId, newComment.trim());
  ...
  return (
    <div className={`playlist-comments ${className}`}>
        ...
        <div className="comments-section">
          ...
          {user ? (
            ...
            <div className="alert alert-info mb-4 d-flex align-items-center">
              <FaComment className="me-2" />
              Please login to comment on this playlist
            </div>
          )}
